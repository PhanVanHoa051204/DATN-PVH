...
public class VoucherController : Controller
{
    ...
    public async Task<IActionResult> Create(Voucher model)
    {
        ...
        model.Code = model.Code.ToUpper().Trim(); // Chuẩn hóa mã viết hoa, xóa dấu cách thừa
    ...
    // 4. Khóa Voucher (Không cho khách dùng nữa)
    [HttpPost]
    public async Task<IActionResult> LockVoucher(int id)
    {
        ...
        if (voucher != null)
        {
            voucher.IsActive = !voucher.IsActive; // Đảo trạng thái (Đang bật -> Tắt, Đang tắt -> Bật)
            await _context.SaveChangesAsync();
    ...
    public async Task<IActionResult> Edit(int id, Voucher model)
    {
        ...
        // Check trùng mã (Lưu ý: Phải loại trừ chính cái Voucher đang sửa ra)
